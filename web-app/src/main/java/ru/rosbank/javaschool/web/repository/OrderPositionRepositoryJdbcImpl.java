...
public class OrderPositionRepositoryJdbcImpl implements OrderPositionRepository {
    ...
    public List<OrderPositionModel> getAll() {
        try {
            return template.queryForList(ds, "SELECT id, order_id, product_id, product_category, product_name, product_price, product_quantity FROM orders_positions;", mapper);
        } catch (SQLException e) {
            throw new DataAccessException(e);
        }
    }
    ...
    public Optional<OrderPositionModel> getById(int id) {
        try {
            return template.queryForObject(ds, "SELECT id, order_id, product_id, product_category, product_name, product_price, product_quantity FROM orders_positions WHERE id = ?;", stmt -> {
                stmt.setInt(1, id);
                return stmt;
            }, mapper);
        } catch (SQLException e) {
    ...
    public void save(OrderPositionModel model) {
        try {
            if (model.getId() == 0) {
                int id = template.<Integer>updateForId(ds, "INSERT INTO orders_positions(order_id, product_id, product_category, product_name, product_price, product_quantity) VALUES (?, ?, ?, ?, ?, ?);", stmt -> {
                    int nextIndex = 1;
                ...
                template.update(ds, "UPDATE orders_positions SET order_id = ?, product_id = ?, product_category = ?, product_name = ?, product_price = ?, product_quantity = ? WHERE id = ?;", stmt -> {
                    int nextIndex = 1;
                    stmt.setInt(nextIndex++, model.getOrderId());
                    stmt.setInt(nextIndex++, model.getProductId());
                    stmt.setString(nextIndex++, model.getProductCategory());
    ...
    public List<OrderPositionModel> getAllByOrderId(int orderId) {
        try {
            return template.queryForList(ds, "SELECT id, order_id, product_id, product_category, product_name, product_price, product_quantity FROM orders_positions WHERE order_id = ?;", mapper, stmt -> {
